...
Bun.js only made sense when it was being used as the web app runtime.  The benefits of Bun.js came from it's API, JavascriptCore, and additional optimizations.  A fully P2P app would not be running on a Node.js/Bun.js style event loop model.  The app would be closer to a single HTML file for an SPA.  This uses the js engine in the browser and loses all the benefits of Bun.js.

The technological _pièce de résistance_ is that the app is P2P, locally stored, and served directly from the user’s device.  From that goal, native app containers and a single file SPA-like web app is crucial.  This leads to a tech combo like Tauri 2 with Vite+.

## New Design

The app is an SPA delivered in a single file with assets embedded.  This can be done with a Vite+ singlefile plugin.  The app runs completely from this single file.  The only other code associated with it will be the native app containers.  The native app containers will be provided by Tauri 2.
